...
{
    struct sll_node * front = ( struct sll_node *)malloc ( sizeof ( struct sll_node ) ) ;
    if ( NULL != front )
    {
        front -> data = data ;
        front -> next = NULL ;
...
struct sll_node * next = node -> next ;
 ...
 void odwroc(struct sll_node** front) {
    struct sll_node * prev = NULL; //Ustawiam anstepny na nulla
    struct sll_node * current = *front; //aktualny jest front
    struct sll_node * next_node = NULL; // i nastepny ustawiam na nulla
