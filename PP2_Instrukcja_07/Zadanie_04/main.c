...
struct vertices_array *convert(int number_of_vertices)
{
    ...
    if (list)
    {
        ...
        {
            printf("Podaj liczbe sasiadow dla wierzcholka %d: ", i);
            ...
            {
                printf("Podaj numer sasiada %d dla wierzcholka %d: ", j, i);
                int neighbour;
                scanf("%d", &neighbour);
                ...
                *new_vertex = (struct vertex *)malloc(sizeof(struct vertex));
                if (*new_vertex)
                {
                    (*new_vertex)->vertex_number = neighbour;
...
void explore_all_vertices(struct vertices_array *adjacency_list, int number_of_vertices, int start_vertex, algorithm_pointer algorithm)
{
...
int main(void)
{
    ...
    adjacency_list = convert(number_of_vertices);
    if (adjacency_list)
    {
        ...
        explore_all_vertices(adjacency_list, number_of_vertices, start_vertex, dfs);
        puts("");
        for (int i = 0; i < number_of_vertices; i++)
            adjacency_list[i].visited = false;
        printf("Wynik algorytmu BFS: ");
        explore_all_vertices(adjacency_list, number_of_vertices, start_vertex, bfs);
        puts("");
